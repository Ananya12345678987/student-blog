...
export default async function FollowingPage({
  ...
  const targetUser = await User.findOne({ username })
    ...
    .populate("following", "name username avatarSeed avatarStyle")
    .lean();

  if (!targetUser) notFound();

  const following = (targetUser as any).following ?? [];
    ...
    <div>
      <div className="mb-6">
        ...
        <h1 className="font-display text-3xl text-ink mt-2">
          {(targetUser as any).name} is Following
        </h1>
      </div>

      {following.length === 0 ? (
        <p className="text-ink/40">Not following anyone yet.</p>
      ) : (
              <ul>
